...
- **Sessão 59 (28/06/2026) — Início da migração planilha → Postgres (unificação c/ Dashboard):**
  ...
  - **Progresso prod:** Próximo: demais Superbet ativas (thuany01, lucielesales03, Parceiro1) + Bet365 ATIVAS (gleice/marlon/Talia) + restantes (Liderbet, Tivo). (gleice/marlon/Talia) pendentes. Betnacional pendente (acima). Base do operador: CSV separado, depois. **Padrão: selecionar por (casa,parceiro), validar stake/PL+colisão, normalizar typos de parceiro ([ Eu]→[Eu]). Arquivo fonte atual: CSV(7).**
