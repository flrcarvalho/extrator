...
_Atualizado: 2026-07-19 (sessão 161 — **Abertas por casa (continuação da 159): BETesporte alinhada.** O Feca pediu estender o "ler apostas EM ABERTO" (feito na Betano) às outras casas passivas. Mapa: **backend + export já são universais** (blindagem do UPSERT e ordenação "abertas no topo" casam por CÓDIGO, que todas têm) → falta só a extensão, casa por casa. **Regra de ouro = código/ID estável** (senão a aberta que fecha não casa e duplica). Estado: **Betano** ✅ (159); **BETesporte** ✅ AGORA (o `formatTicketBE` já tratava aberta via `openBetsCount`; só alinhei o texto do status p/ o sinal inequívoco "aguardando resultado — NÃO liquidar; sem resultado", igual à Betano, pra IA nunca chutar resultado); **Superbet** 🟡 e **Betfair** 🔴 PENDENTES (o Feca vai capturar uma amostra da API de aposta aberta de cada — preciso do valor exato do status "aberto", que não sei sem ver ao vivo; chutar arriscaria classificar um `void` como aberta); **Bet365** ⛔ EXCLUÍDA (sem ID nem data → aberta que fecha viraria duplicata fantasma). `node --check` verde; manifest 0.3.5→0.3.6 (**recarregar a extensão**). Backup `Backups/pre_betesporte_abertas_2026-07-19/`. Verificação ao vivo na aba "Em aberto" da BETesporte pendente. Ver [[betano_abertas_e_upsert]] · [[extensor_captura]].)_
