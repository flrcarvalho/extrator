...
- **Sessão 59 (28/06/2026) — Início da migração planilha → Postgres (unificação c/ Dashboard):**
  ...
  - **Progresso prod:** Falta SÓ gleicecacia01 [Richard] (Bet365 ativa, ~1.435 antigas) p/ fechar a base do Feca. Depois: base do operador (CSV separado). (gleice/marlon/Talia) pendentes. Betnacional pendente (acima). Base do operador: CSV separado, depois. **Padrão: selecionar por (casa,parceiro), validar stake/PL+colisão, normalizar typos de parceiro ([ Eu]→[Eu]). Arquivo fonte atual: CSV(7).**
