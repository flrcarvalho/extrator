...
- **Sessão 59 (28/06/2026) — Início da migração planilha → Postgres (unificação c/ Dashboard):**
  ...
  - **Próximo:** continuar na ordem da planilha (próximo bloco = Betano, ~3.7k, também split inativos/ativos por parceiro). As 3 Bet365 ATIVAS + demais ativas ficam pra passada final com era-split/backfill (tipster+odd+resultado). Base do operador entra depois (CSV separado).
